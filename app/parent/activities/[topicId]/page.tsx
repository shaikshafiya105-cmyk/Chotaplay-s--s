...
export default function ParentActivityVideoPage() {
  ...
  useEffect(() => {
    ...
    async function verifyAccess() {
      ...
      const allowed = await checkTopicAccess('activities', topic!.id);
      if (mounted) {
        if (!allowed) {
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col justify-center">
      <StandardVideoPlayer
        topic={topic}
        backToTopicsHref="/parent/activities"
        gameHref={topic.hasGame ? `/parent/activities/${topic.slug}/game` : undefined}
      />
    </main>
  );
}
