...
export default function ExploreTopicGridPage() {
  ...
  useEffect(() => {
    ...
    async function loadStates() {
      try {
        const states = await fetchGradeLockStates('explore');
        if (mounted) {
          setLockStates(states);
          setLoading(false);
        }
      } catch (e) {
        console.error('Failed to load explore lock states', e);
